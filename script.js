/* MM_STABLE_RUNTIME: lightweight runtime safeguards */
(function(){
  window.MM_STABLE_RUNTIME = true;
  try {
...
/* ===== ORDER CALCULATOR ===== */
let calcItems=[];
const calcProduct=document.getElementById("calcProduct"),calcQty=document.getElementById("calcQty"),
calcList=document.getElementById("calcList"),calcTotal=document.getElementById("calcTotal");
function initCalculator(){
  if(!calcProduct)return;
  calcProduct.innerHTML=products.map(p=>`<option value="${p.id}">${p.name} — ${p.price} сом</option>`).join("");
  renderCalculator();
}
function renderCalculator(){
  if(!calcList)return;
  let total=0;
  calcList.innerHTML=calcItems.length?calcItems.map((x,i)=>{
    const p=products.find(q=>q.id===x.id); if(!p)return "";
    const sum=p.price*x.qty; total+=sum;
    return `<div class="calc-row"><b>${p.name}</b><span>${x.qty} × ${p.price}</span><strong>${sum.toLocaleString("ru-RU")} сом</strong><button data-calc-remove="${i}">✕</button></div>`;
  }).join(""):'<div class="empty-drawer">🧮<br><br>Эсептөө үчүн товар кошуңуз.</div>';
  calcTotal.textContent=total.toLocaleString("ru-RU")+" сом";
}
document.addEventListener("click",e=>{
  const cr=e.target.closest("[data-calc-remove]");
  if(cr){calcItems.splice(+cr.dataset.calcRemove,1);renderCalculator();}
});
if(document.getElementById("calcAdd"))document.getElementById("calcAdd").onclick=()=>{
  const id=+calcProduct.value,qty=Math.max(1,+calcQty.value||1);
  const old=calcItems.find(x=>x.id===id); if(old)old.qty+=qty; else calcItems.push({id,qty});
  renderCalculator();calcQty.value=1;
};
if(document.getElementById("calcClear"))document.getElementById("calcClear").onclick=()=>{calcItems=[];renderCalculator();};
if(document.getElementById("calcOrder"))document.getElementById("calcOrder").onclick=()=>{
  if(!calcItems.length){alert("Алгач товар кошуңуз.");return;}
  let total=0,msg="Саламатсызбы! Мой Маркеттен эсептелген заказ:%0A%0A";
  calcItems.forEach(x=>{const p=products.find(q=>q.id===x.id);const s=p.price*x.qty;total+=s;msg+=`• ${encodeURIComponent(p.name)} — ${x.qty} даана — ${s} сом%0A`;});
  msg+=`%0AЖалпы: ${total} сом`;
  window.open(`https://wa.me/${WA}?text=${msg}`,"_blank");
};
initCalculator();
